...
## Lite mode
During discovery, pm asks: small tool or real product? Small tool -> lite mode: `lead` (pm+tech-lead combined, model opus, `.claude/agents/lead.md`) + dev + qa; designer/release stay conditional; reviewer runs ONCE at closure instead of per-increment. Real product -> full pipeline below.
...
- Phase 2 — Design: tech-lead (or lead) writes design.md + increment plan (INC-1..N), each with explicit acceptance criteria dev can self-verify before handoff. Questions route to pm; user-level decisions route to the user via pm. Designer writes ux-spec.md in parallel if user-facing UI exists.
  Before requesting GATE 3, tech-lead runs two self-checks: (1) every FR/NFR ID an increment's plan claims has at least one matching AC in that increment's own AC list — an FR/NFR claimed with zero matching AC is a design-phase defect QA should never be first to discover; (2) every NFR/AC that requires live network access, external services, or browser automation the current environment cannot reach is identified and routed to the user via pm once, as an explicit go/no-go — defer now with sign-off, or provide a reachable environment before increments start. Don't let each increment rediscover and re-disclose the same environment gap independently.
...
- Phase 3 — Increment loop (full pipeline), for each INC-N (vertical slice: shippable, working end-to-end on the previous merge):
  ...
  d. reviewer runs a diff-scoped audit: only files changed since the last reviewer clearance (`git diff --name-only <last-cleared>..HEAD`), plus traceability of the FR/NFR IDs the increment claims, plus a re-read of the immediately prior increment's docs/handoff.md "Known limitations" and docs/test-report.md "Flagged for routing" notes — any gap named in two consecutive increments without an existing finding ID gets logged now, not deferred to the closure audit. Blockers route to owning agent before next increment.
...
## Orchestrator rules
- **Agent model enforcement (CRITICAL)**: The Orchestrator runs on Haiku 4.5; every subagent MUST explicitly receive the `model:` parameter matching its `.claude/agents/<type>.md` frontmatter. pm/tech-lead/lead use `"opus"`; designer/dev/qa/reviewer use `"sonnet"`; release uses `"haiku"`. See `.claude/orchestrator-checklist.md` — this is non-negotiable; violations result in underperforming agents and incomplete work.
...
- No acceptance criterion for rendering, layout, accessibility, computed styling, or non-technical-user-editability may be satisfied by a source/static check alone — it requires a live-artifact verification method (rendered screenshot, computed-style read, or scripted click-through against a running instance). "Cannot be verified in the current environment" halts the increment and routes to the user via pm; it is never grounds for a static-only substitute or a version deferral.
